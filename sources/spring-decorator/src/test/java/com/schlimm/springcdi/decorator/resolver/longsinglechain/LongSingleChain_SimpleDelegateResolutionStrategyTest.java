...
public class LongSingleChain_SimpleDelegateResolutionStrategyTest {
	...
	public void testChaining_MyDecorator3LeadsToMyDelegate() {
		DecoratorInfo decoratorInfo = null;
		try {
			decoratorInfo = new DecoratorInfo("myDecorator3", beanFactory.getBeanDefinition("longSingleChain_MyDecorator3"), Class.forName("com.schlimm.springcdi.decorator.resolver.longsinglechain.LongSingleChain_MyDecorator3"));
		} catch (NoSuchBeanDefinitionException e) {
			TestCase.fail(e.getMessage());
		} catch (ClassNotFoundException e) {
			TestCase.fail(e.getMessage());
